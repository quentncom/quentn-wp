...
class Quentn_Wp_Restrict_Access
{
	...
	public function quentn_content_permission_check( $content ) {
		...
		if ( $page_meta['redirection_type'] == 'restricted_url' && $page_meta['redirect_url'] != '' ) {
			...
			$error_message = ! empty( $page_meta['error_message'] ) ? $page_meta['error_message'] : '<h3>' . __( 'Access denied', 'quentn-wp' ) . '</h3>';
			return apply_filters( 'quentn_access_denied_error_message', $error_message );
		}
	}
	...
	public function set_replacement_values() {
		...
		if ( isset( $_GET['qntn'] ) ) {
			...
			if ( isset( $qntn_values['email'] ) && is_email( sanitize_email( $qntn_values['email'] ) ) && count( $qntn_values ) > 1 ) {
				...
				global $wpdb;
				//add it in database
				$wpdb->replace( $wpdb->prefix . TABLE_QUENTN_USER_DATA, [ 'email'  => $email,
				                                                          'fields' => serialize( $qntn_values )
				], [ '%s', '%s' ] );
			}
    ...
    public function get_new_access() {
		...
		if ( isset( $_GET["qntn_wp"] ) ) { //get email address if it is in qntn query string
		...
		if ( $created_at && $quentn_page_restrict_meta['countdown'] ) {
			...
			if ( $quentn_page_restrict_meta['countdown_type'] == 'absolute' ) {
				...
				if ( isset( $quentn_page_restrict_meta['absolute_date'] ) && strtotime( $quentn_page_restrict_meta['absolute_date'] ) !== false ) {
					$remaining_time = $this->calculate_absolute_page_expire_time( $quentn_page_restrict_meta['absolute_date'] );
				}
				...
				$remaining_time = $this->calculate_relative_page_expire_time( $created_at );
			}
		}
